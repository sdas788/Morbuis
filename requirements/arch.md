...
### Automation Planner (E-028) — backfilled 2026-06-10

**Purpose:** authoritative card↔flow mapping per project so cards run without committing machine-specific Maestro paths. Surfaces draft flows (`data/{projectId}/flows/*.yaml`) in an "Automation Plan (drafts)" category, marks cards blocked-with-reason, and carries persona context.

**Data model:** Automation-Planner + persona-registry types in `src/types.ts`; persistence in `src/parsers/markdown.ts`. Credential **values** live in per-project `env`, never in the committed plan (plan references persona keys only).

**Run path:** `resolvePlannedFlowPath` + the `/api/test/run` fallback resolve a planned flow id to `data/{projectId}/flows/{flowId}.yaml` so a planned-but-uncommitted flow is one-click runnable. `blockedMap` (built in `loadMorbiusData`) drives the ⛔ badge. Same resolution feeds true per-card automation coverage. UI: `AutomationPlanView`.

### QA Plan — Orientation Layer + Curate (E-029) — backfilled 2026-06-10

**Purpose:** bring the product team's roadmap *above* the test cases (QA Plan, Flow Plans TF-NNN, Release Plans TR-NNN, User Flows UF-NNN) and make the board legible + honest for the QA team. PMAgent owns *planned*; Morbius owns *passing*.

**Import:** `importPMAgentPlans` / `parsePlanDoc` / `loadPlansIndex` / `extractPlanSections` copy `qa/` + `releases/` + user-flow docs verbatim into `data/{projectId}/plans/` on the same one-click push as the cases. Routes: `/api/plans`, `/api/plans/:id` (returns body + parsed sections; traversal guard `^[A-Za-z0-9_.-]+\.md$`).

**Render:** `PlansView` / `PlanDetail` / `SectionCard` (progressive disclosure); `MermaidBlock` renders User-Flow screen-flow graphs with a zoom/pan viewport. Execution overlay: `planExecRollup` + `ExecBar` roll up Morbius card status across a plan's `S-NNN-NNN` stories (per-plan + per-release readiness — the truth PMAgent's QA tab can't show).

**Curate:** Triage Shelf (`reviewEpics` on `ProjectConfig`) quarantines raw/meta epics; `recomputeSourceChecksum` (`parsers/pmagent.ts`) badges In sync / Drifted / Source missing; `splitACs` / `deriveTitle` fix shredded titles at the source. Enforced by `morbius doctor` legibility checks + the `morbius-curate` skill + a Curate stage in `requirements/HARNESS.md`. Round-trip nav via `handleNavigate` / `focusStory|Flow|Plan` / `navTo`.
...
| 2026-06-10 | 1.11 | Claude | **Board drift correction.** Backfilled E-028 (Automation Planner, 5 stories) + E-029 (QA Plan Orientation Layer + Curate, 8 stories) — both shipped in code without a board epic. Created R-003 (June Wave) homing E-023–E-029. Flipped stale epic stages to match code: E-001–006 + E-013–017 + E-024 + E-027 → Done, E-023 + E-025 → In Progress, E-021 → Parked (re-confirmed parked, never built). R-002 → In Progress. Added arch sections for E-028/E-029 above. |
