...
5. **Agents — v2.1 cloud runner targets Anthropic Managed Agents.** *(v1.1, 2026-04-30 — was: "do NOT pull in Claude Agent SDK or OpenAI SDK until E-022 gate criteria are met". E-022 flipped to `decided` on 2026-04-30; see E-026 for implementation.)* Claude Agent SDK is the documented fallback if Managed Agents doesn't fit the browser-MCP-locality constraint (see arch.md C6 / E-025 Constraints / E-026 S-026-002). CLI subprocess remains the local-laptop default and the v2.0 cloud runner.
...
| 2026-04-30 | 1.1 | Claude | Guardrail #5 reframed. E-022 gate flipped to `decided` (migrate to Anthropic Managed Agents as v2.1). v2.0 cloud deploy (E-025) ships with Claude CLI subprocess unchanged; v2.1 implementation lives in E-026 with a 1-week derisking spike on browser-MCP locality (Constraint C6). |
